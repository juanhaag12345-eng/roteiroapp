// Otimizacao de rota (problema do caixeiro-viajante simplificado) + guia de
// navegacao turn-by-turn quando o OSRM esta disponivel.
//
// Estrategia:
//  1) Tenta usar o servico publico OSRM (Trip API, com steps=true) para obter
//     a ordem otima, o desenho da rota nas ruas e as instrucoes de manobra
//     (vire a esquerda, siga em frente, etc.) para cada trecho.
...
//     mas sem instrucoes de manobra (o guia de navegacao usa so a distancia
//     em linha reta ate cada parada nesse caso).
...
function haversineFallback(origin, stops) {
  ...
  return {
    ...
    source: 'haversine',
    steps: [], // sem instrucoes de manobra nesse modo (sem acesso a malha viaria real)
  };
}
...
async function osrmTrip(origin, stops) {
  ...
  // Cada leg do trip corresponde a um trecho ja na ordem otimizada (origem ->
  // 1a parada, 1a parada -> 2a parada, ...). Cada leg tem varios steps com a
  // instrucao de manobra (tipo + modificador + nome da rua).
  const steps = [];
...
async function optimizeRoute(origin, stops) {
  if (!stops.length) {
    return { order: [], geometryLatLng: [origin], distanceKm: 0, durationMin: 0, source: 'none', steps: [] };
  }
  try {
