...
const statusRastreio = document.getElementById('statusRastreio');
...
// ---------------- rastreamento em tempo real (sempre ativo) ----------------
// Nao ha botao de ligar/desligar: assim que um vendedor e identificado nesta
// tela, o rastreamento por GPS comeca automaticamente e fica ativo enquanto
// a aba estiver aberta, para o gestor poder acompanhar quando quiser.
...
function iniciarRastreioAutomatico(vendedorId) {
    pararRastreio();
    if (!vendedorId || !statusRastreio) return;

  if (!('geolocation' in navigator)) {
        statusRastreio.className = 'erro';
        statusRastreio.textContent = 'Geolocalizacao nao suportada neste navegador - o gestor nao conseguira ver sua localizacao.';
        return;
  }

  vendedorRastreado = vendedorId;
    statusRastreio.className = 'aviso';
    statusRastreio.textContent = 'Ativando rastreamento...';
  ...
  const s = garantirSocket();
    watchId = navigator.geolocation.watchPosition(
      ...
      const { latitude: lat, longitude: lng } = pos.coords;
                  ...
                  statusRastreio.className = 'sucesso';
                  statusRastreio.textContent = 'Rastreamento ativo - sua localizacao esta sendo compartilhada com o gestor.';
          },
          (err) => {
                  statusRastreio.className = 'erro';
                  statusRastreio.textContent = `Erro de GPS: ${err.message}. Sem a permissao de localizacao do navegador, o gestor nao ve sua posicao.`;
          },
