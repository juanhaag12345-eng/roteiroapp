...
function origemIcon() {
  return L.divIcon({
    className: '',
    html: `<div style="width:30px;height:30px;border-radius:50%;background:#1a1a1a;color:#fff;display:flex;align-items:center;justify-content:center;font-size:15px;box-shadow:0 1px 4px rgba(0,0,0,.4)">🏠</div>`,
    iconSize: [30, 30],
    iconAnchor: [15, 15],
  });
}
...
function iniciarRastreioGPS(vendedorId, dia) {
  if (!('geolocation' in navigator)) {
    document.getElementById('msgRastreio').innerHTML = '<div class="erro">Geolocalização não suportada neste navegador.</div>';
    return;
  }
  ...
  watchId = navigator.geolocation.watchPosition(
    (pos) => {
      ...
      s.emit('vendedor:location', { vendedorId, lat, lng, tracking: 'gps', dia });
    },
    (err) => {
      document.getElementById('msgRastreio').innerHTML = `<div class="erro">Erro de GPS: ${err.message}. Verifique a permissão de localização do navegador.</div>`;
    },
    { enableHighAccuracy: true, maximumAge: 5000, timeout: 15000 }
  );
}

function iniciarSimulacao(vendedorId, dia) {
  if (!rotaAtual || !rotaAtual.geometryLatLng || rotaAtual.geometryLatLng.length < 2) {
    document.getElementById('msgRastreio').innerHTML = '<div class="erro">Trace uma rota primeiro.</div>';
    return;
  }
  const s = garantirSocket();
  const pontos = rotaAtual.geometryLatLng;
  let i = 0;
  simTimer = setInterval(() => {
    ...
    const p = pontos[i];
    marcarMinhaLocalizacao(p.lat, p.lng);
    s.emit('vendedor:location', { vendedorId, lat: p.lat, lng: p.lng, tracking: 'sim', dia });
    i++;
  }, 1500);
}
...
function pararRastreio() {
  ...
  const vendedorId = selVendedor.value;
  if (socket && vendedorId) socket.emit('vendedor:offline', { vendedorId });
  document.getElementById('btnIniciar').disabled = false;
  document.getElementById('btnParar').disabled = true;
  document.getElementById('msgRastreio').innerHTML = '<p class="small">Rastreamento parado.</p>';
}
...
document.getElementById('btnIniciar').addEventListener('click', () => {
  ...
  const dia = selDia.value;
  if (!vendedorId) return;
  const modo = document.querySelector('input[name="modoRastreio"]:checked').value;
  document.getElementById('btnIniciar').disabled = true;
  document.getElementById('btnParar').disabled = false;
  document.getElementById('msgRastreio').innerHTML = '<p class="small">Rastreando… mantenha esta aba aberta.</p>';
  if (modo === 'gps') iniciarRastreioGPS(vendedorId, dia);
  else iniciarSimulacao(vendedorId, dia);
});

document.getElementById('btnParar').addEventListener('click', pararRastreio);
